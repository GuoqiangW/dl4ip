...
class DiceCoefficient(object):
    ...
    def update(self, pred, target):
        ...
        # pred : [b, c, w, h] ， 将pred转为真实0/1类别
        pred = torch.as_tensor((pred > 0.5), dtype=torch.int64)
        target = torch.as_tensor((target > 0.5), dtype=torch.int64)
        pred = F.one_hot(pred.squeeze(dim=1), self.num_classes).permute(0, 3, 1, 2).float()
        dice_target = build_target(target.squeeze(dim=1), self.num_classes, self.ignore_index)
